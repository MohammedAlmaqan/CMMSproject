    ...
    <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#111113' }}>
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="industrial-card rounded-lg p-6">
          <div className="mb-4">
            <label className="block text-secondary text-xs font-medium mb-1.5">
              Username
            </label>
            <select
              value={username}
          ...
          {error && (
            <div className="mb-4 text-red-status text-xs">{error}</div>
          )}

          <button
            type="submit"
            className="w-full py-2 rounded text-sm font-semibold transition-all hover:brightness-110"
            style={{
        ...
        <p className="text-center text-tertiary mt-4" style={{ fontSize: '11px' }}>
          Select any user — password is not validated for demo
        </p>
      </div>
    </div>
  );
