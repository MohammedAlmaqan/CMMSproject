...
| §3.3.2 | System must enforce state transitions based on user roles (e.g. only Supervisor may Close) | Not Met | `backend/src/routes/workOrders.ts:489` (`router.put('/:id/status', authorizeMinRole('Technician'), ...)`) | The single status endpoint is gated at Technician for **all** transitions, and no per-transition role check exists. A Technician can call `PUT /api/work-orders/:id/status` with `Closed` and succeed. The Close control is hidden below Supervisor only in the UI (`app/src/pages/WorkOrderDetailPage.tsx`), which is not enforcement |
...
| §3.3.7 | WO cannot be set to “In Progress” unless all mandatory safety checklists are acknowledged (sign-off via electronic signature) | Not Met | `backend/src/routes/workOrders.ts:489-536` — the status handler contains **no** reference to `checklist`, `isMandatory`, or sign-off | **Safety-critical gap.** A work order with an unacknowledged mandatory Lockout/Tagout or Confined Space checklist can be moved to In Progress, and the audit log will record a normal status change. This is the single highest-severity finding in this audit |
...
|---|---|---|---|---|
| §6.4 | All functional requirements listed in §3 are implemented and pass UAT scripts | Not Met | This matrix (§3.1–§3.10) | Of the 126 §3 requirements, 37 are Not Met and 41 are Partial; no §3 subsection is fully satisfied. No UAT script pack exists — `scripts/verify/` holds implementation-verification scripts, not business UAT |
...
| §6.4 | No open Critical or Major defects at go-live | Not Met | This matrix; `docs/WCAG-AUDIT.md` | The mandatory-checklist bypass (§3.3.7) and the Technician-can-Close authorization gap (§3.3.2) are both release-blocking, and the WCAG audit carries unresolved findings |
...
| L10 | The Close transition is gated at Technician server-side, so role enforcement does not match the documented policy | §3.3.2 | `backend/src/routes/workOrders.ts:489` | **V1.1-RBAC** — per-transition role policy driven by the SOW transition table, with Supervisor-or-above required for Closed |
| L11 | Work orders can be moved to In Progress with unacknowledged mandatory safety checklists | §3.3.7 | `backend/src/routes/workOrders.ts:489-536` (no checklist check) | **V1.1-SAFETY** — block the In Progress transition server-side until every mandatory checklist is acknowledged by the responsible technician, and record the acknowledgement in the audit log |
...
Fully Met across 63 of 214 clauses (29%). The §3 functional requirements — the core of the SOW — are 37 Met, 41 Partial, and 37 Not Met out of 126, so **no §3 subsection is fully satisfied end to end**.

**Requirements that block go-live** (Not Met, no Client deferral or exclusion):

1. **§3.3.7 — mandatory safety checklists do not gate “In Progress”.** A work order with an unacknowledged Lockout/Tagout or Confined Space checklist can be started. This is a personnel-safety control and must be fixed before any production use.
2. **§3.3.2 — the Close transition is not role-restricted server-side.** A Technician can close any work order through the API.
3. **§3.3.5 — labour time is attributed from a client-supplied user ID**, so recorded labour cost and traceability can be falsified from the browser.
4. **§3.2.1 / §3.2.3 — M3 notifications are never generated and notification status never follows work order completion**, breaking the lifecycle the SOW specifies.
5. **§6.4 — no UAT pack exists and no report has been reconciled against a manual calculation**, so the acceptance criteria cannot currently be evidenced.
