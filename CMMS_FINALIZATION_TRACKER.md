...
| 6.2 | Scheduled `pg_dump` backup + documented restore drill | ✅ | Daily 02:00 schedule + newest-14 retention documented. Initial drill PASS: 306,826-byte dump, `WorkOrder` count 84, elapsed 5.35 s, `cmms_restore_test` absent after drop (`psql -l \| findstr` exit 1). RPO ≤ 24 h; measured RTO 5.35 s. A.1 current-script re-drill: `PASS: dump restored, WorkOrder query succeeded, drill database dropped.`, exit 0. PostgreSQL server references aligned to 15+; client detection is PATH first, then the documented PostgreSQL 18 local fallback. | this commit |
