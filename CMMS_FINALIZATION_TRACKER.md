...
| 2.2 | Load remaining collections live: users, crafts, taskLists, operations, labor, checklists, meter readings, comments, **audit log** | ✅ | Administration audit tab + crafts + users live via loadFromApi; WODetail tabs live; taskLists/meterReadings store fields stay empty (no wired endpoint) — pages honest-empty, zero fabrication | 40e90af, d842bb9, 9876e67 |
...
| 2.9 | Apply `authorizeMinRole` per SOW role matrix | 🔶 | WO-domain RBAC confirmed (Milestone A); SOW-matrix sweep for remaining routers deferred to Phase 5 (G7 Administration) | 367b59d |
| 2.10 | Zod validation on all request bodies/params | 🔶 | Schemas centralized in `validation.ts` (WO, notifications, equipment, locations, meters, checklists); full-route coverage sweep deferred to Phase 5 | 367b59d |
