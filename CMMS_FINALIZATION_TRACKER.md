...
**Status:** Phase 0 - Complete | Phase 1 (1.0–1.10) - Complete | Phase 2 - In Progress (Milestone A: WO domain E2E - complete; Milestone B Group 1: WO sub-domain CRUD + costs + board - complete; Group 2: Notifications complete; **Group 3: Asset Master complete; Group 4a: Preventive Maintenance frontend + API complete (F1/F2 recorded); Group 4b: PM scheduler complete (3.1a–3.1g ✅); Group 5: Dashboard + Reports complete (2.4/2.5/2.6 ✅); **Group 6a: mockData deletion + fallback removal complete (2.3 ✅, trust property); Group 6b pending**)
...
Note (tracked tech-debt, Phase 0.5 -> Phase 5): `tsc -b` (the real gate) reports exactly 4 errors, all in page code untouched by G6a — `EquipmentDetailPage.tsx(184,72)` TS2551 `EquipmentBOM.material` (did you mean `materialId`?), `WorkOrderDetailPage.tsx(505,11)` + `(533,11)` TS2322 `string | null` vs `string | undefined`, and `(857,43)` TS2345 `string` vs `SetStateAction<DetailTab>`. Proven not introduced by G6a: `git diff 58d7ef8..9876e67 -- <both files>` is empty, and a worktree checked out at pre-G6a `58d7ef8` reproduces these 4 plus a 5th (`appStore.ts(256,94)`) that G6a fixed — so `tsc -b` error count strictly decreased 5->4. Last file edits predate the deletion (`d842bb9`/`c40030a`/`4c3662c`, G1/G3 era). Fix target: Phase 5. Note that Phase 0.5 recorded `frontend-tsc` exit 0 via the literal `tsc --noEmit -p app` command, which compiles nothing (project root tsconfig has only `references`); the failing-but-meaningful gate is `app`'s `tsc -b`.
