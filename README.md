...
- **The Settings screen shows values that do not match runtime behaviour.** It is hardcoded and already wrong in places — it advertises a "30 minutes" session timeout while the JWT actually lives for 8 hours, and lists an audit retention window and upload limit that are not enforced in code. Do not treat it as documentation of the running system.
